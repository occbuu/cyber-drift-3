...
class Model(object):
    ...
    def build_lstm_model(self):
        ...
        #--------Build LSTM------
        n_hidden = 512
        rnn_cell = tf.keras.layers.LSTMCell(n_hidden)
    ...
    def build_model(self):
        ...
        # self.output = tf.layers.dense(self.output, n_classes, activation=tf.nn.tanh)
        ...
        for epoch in range(epochs):
            ...
            while 1:
                ...
                feed_dict_batch = {self.x: x, 
                                   self.y: y}
                _, batch_loss, output, test= self.sess.run((self.optimizer, self.loss, self.output, self.test), feed_dict=feed_dict_batch)
                # if counter % 20 == 0:
                    # print(test.shape)
                    # print(counter)
                    # print(y)
    ...
    def post_train(self, dataController):
        ...
        for epoch in range(epochs):
            ...
            while 1:
                ...
                _, batch_loss, output, test= self.sess.run((self.post_optimizer, self.post_loss, self.output, self.test), feed_dict=feed_dict_batch)
                # print(batch_loss)
                # print(output)
                # print(test)
                epoch_losses.append(batch_loss)
        ...
        while 1:
            ...
            if mode == 'DOC':
                ...
                for i in range(self.batch_size):
                    ...
                    self.logger.info("Truth: {}    DOC: {}".format(y[i], doc_predict)) 
                ...
                for i in range(self.batch_size):
                    ...
                    self.logger.info('openmax_scores: {}'.format(openmax))
                    self.logger.info('softmax_scores: {}'.format(softmax))
                    self.logger.info('OpenMax: {}    Softmax: {}'.format(openmax_predict, soeftmax_predict))
                    self.logger.info('')
                    self.logger.info("Truth: {}    OpenMax: {}     Softmax: {}".format(\
